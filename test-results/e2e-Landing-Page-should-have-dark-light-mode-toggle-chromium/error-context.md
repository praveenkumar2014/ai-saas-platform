...
- Name: e2e.spec.ts >> Landing Page >> should have dark/light mode toggle
- Location: tests/e2e.spec.ts:22:7
...
Locator: locator('button').filter({ hasText: /dark|light/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: /dark|light/i })
